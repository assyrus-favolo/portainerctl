...
These override the config file for scripting and CI:
...
# Full kubectl passthrough (requires kubectl installed)
...
All list commands produce tabular output by default. All get/inspect commands produce JSON, which is directly pipeable to `jq`:

```bash
portainerctl env list
portainerctl stack get 5 | jq '.GitConfig.URL'
portainerctl user get 1 | jq '.Role'
portainerctl activity auth-logs | jq '.[] | select(.Action == "user.login.failure")'
```
...
# Tagged release (run from GitHub Actions via the included workflow)
git tag v0.1.0
git push origin v0.1.0
```

The GitHub Actions workflow at `.github/workflows/release.yml` handles this automatically on any tag push.
...
The CLI will work against any BE instance at or near this version. CE is not supported — BE-only endpoints (licensing, RBAC roles, edge compute, policies, observability, cloud credentials) will return 403 on CE.
